...
import backIcon from '../../assets/21_1557.svg'
import po2Icon from '../../assets/51_77.svg'
import po1Icon from '../../assets/51_80.svg'
import po3Icon from '../../assets/51_83.svg'
...
// Format tanggal ISO ke format Indonesia (contoh: 23 Agustus 2026, 08:47)
...
// Format angka tanpa prefix Rp (contoh: 1.000.000)
...
const poIcons = [po1Icon, po2Icon, po3Icon]

function RiwayatBeliProdukPage({ onBackClick }) {
  ...
  const loadTransactions = async (pageNum, append = false) => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(getErrorMessage(data, 'Gagal memuat riwayat beli produk.'))
        return
      }
  ...
  return (
    <div className="min-h-screen bg-background font-sans">
      ...
      <section className="bg-background min-h-screen pb-10 flex justify-center">
        <div className="w-full max-w-md bg-white px-4 py-2 shadow-sm">
          {loading ? (
            <p className="text-textLight text-sm text-center py-10">
              Memuat riwayat beli produk...
            </p>
          ) : transactions.length === 0 ? (
            <p className="text-textLight text-sm text-center py-10">
              Belum ada riwayat beli produk.
            </p>
          ) : (
            transactions.map((item, index) => (
              <div
                key={item.id ?? item.trx_id ?? index}
                className="flex items-center py-3.5 border-b border-borderGray last:border-b-0 gap-3 hover:bg-gray-50 transition-colors cursor-pointer"
              >
                <div className="w-10 h-10 rounded-full bg-[#dbe9fb] flex items-center justify-center shrink-0">
                  <img
                    src={poIcons[index % poIcons.length]}
                    alt="Trending up"
                    className="w-4 h-4"
                  />
                </div>
                <div className="flex-1 flex flex-col gap-0.5">
                  <h2 className="text-textDark text-sm font-medium">
                    Pembelian {item.product_name || 'Produk'}
                  </h2>
                  <p className="text-textLight text-xs">
                    {formatDate(item.created_at)}
                  </p>
                </div>
                <div className="text-[#d6392c] text-sm font-medium">
      ...
      <ErrorModal
        open={Boolean(error)}
        title="Gagal Memuat Riwayat Beli Produk"
        message={error}
        onClose={() => setError('')}
      />
    </div>
...
export default RiwayatBeliProdukPage
